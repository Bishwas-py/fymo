"""$auth client module emission (issue #80 phase 4).

Mirrors the $remote/$broadcast codegen conventions: a .js module plus a
.d.ts at dist/client/_auth.{js,d.ts}, dependency-free apart from svelte/store
...
def test_emits_js_and_dts(tmp_path: Path):
    ...
    js = (tmp_path / "client" / "_auth.js").read_text()
    dts = (tmp_path / "client" / "_auth.d.ts").read_text()
