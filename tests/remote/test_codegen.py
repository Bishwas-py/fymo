...
def test_runtime_error_branch_prefers_message_over_error_code(tmp_path: Path):
    """Regression: the generated client must surface the server's actual
    `message` (e.g. "Agent exploration failed: ...") to a caller's `catch`,
    not just the short error code (e.g. "internal") — the code alone was
    useless for a user or developer to act on, and the real message was
    being computed server-side and sent over the wire, then silently
    dropped by this exact line before app code ever saw it."""
    from fymo.remote.codegen import emit_runtime
    emit_runtime(tmp_path)
    runtime = (tmp_path / "__runtime.js").read_text()
