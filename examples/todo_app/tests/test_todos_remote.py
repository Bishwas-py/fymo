"""Tests for app/remote/todos.py (generated; they are yours).
...
def test_list_todos_returns_the_seed_row():
    items = list_todos()
    assert any(item["created_by"] == "seed" for item in items)


def test_get_todo_returns_the_seed_row():
    assert get_todo(1)["created_by"] == "seed"


def test_get_todo_unknown_id_raises_not_found():
    with pytest.raises(NotFound):
        get_todo(999)


def test_create_todo_attributes_the_signed_in_caller():
    with signed_in("u_test1") as ident:
        item = create_todo(title="A new item")
    assert item["title"] == "A new item"
    assert item["created_by"] == ident.uid


def test_create_todo_rejects_anonymous_callers():
    with request_scope(uid="u_anon", environ={}):
        with pytest.raises(RemoteError) as excinfo:
            create_todo(title="nope")
    assert excinfo.value.status == 401


def test_update_todo_by_the_owner():
    with signed_in("u_test1"):
        item = create_todo(title="Draft")
        updated = update_todo(item["id"], title="Final")
    assert updated["id"] == item["id"]
    assert updated["title"] == "Final"


def test_delete_todo_by_the_owner():
    with signed_in("u_test1"):
        item = create_todo(title="Disposable")
        deleted = delete_todo(item["id"])
    assert deleted["id"] == item["id"]
    assert all(row["id"] != item["id"] for row in list_todos())


def test_someone_elses_todo_reads_as_missing():
    ...
    with signed_in("u_test1"):
        item = create_todo(title="Mine")
        with pytest.raises(NotFound):
            update_todo(1, title="steal the seed row")
        with acting_as("u_other"):
            with pytest.raises(NotFound):
                update_todo(item["id"], title="steal")
            with pytest.raises(NotFound):
                delete_todo(item["id"])
